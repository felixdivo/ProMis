...
class Map(ABC):
    ...
    def filter(self, location_type: str | set[str]) -> DerivedMap:
        """Get a map with only features of the given type or types.

        Args:
            location_type: The type(s) of locations to filter for

        Returns:
            A map that only contains features of the given type(s)
        """
        location_types = location_type if isinstance(location_type, set) else {location_type}

        return type(self)(
            self.origin,
            [feature for feature in self.features if feature.location_type in location_types],
        )
    ...
    def apply_covariance(self, covariance: ndarray | None):
